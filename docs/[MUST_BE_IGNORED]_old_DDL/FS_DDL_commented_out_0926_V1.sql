/****************************************************************************************
 * DDL CONTEXT DOCUMENT
 * * ## 目的
 * このDDLは、財務モデリングアプリケーションの中核をなすデータベーススキーマを定義します。
 * 別の開発担当LLMがこのスキーマ定義を読み解き、コアロジックを実装するための技術仕様書として機能します。
 * * ## 背景・議論の経緯
 * 当初、計算ロジックを格納するテーブルは複数に分かれていましたが、議論を経て、
 * 全ての計算ルールを `calculation_rules` テーブルに一元管理する方針となりました。
 * これにより、計算エンジンの参照先が単一になり、システムの複雑性が大幅に軽減されました。
 * また、「1つの勘定科目に、1つの期間、1つのルール」という大原則を設け、これをDBのUNIQUE制約で
 * 保証することで、システムの予測可能性と堅牢性を高めています。
 * * ## このDDLが目指す状態
 * - 財務モデルの階層構造（Project > Model > Scenario）が明確に表現されている。
 * - ユーザーが定義する勘定科目（user_accounts）と、システムが標準で持つ勘定科目（global_accounts）が分離されている。
 * - 全ての計算ロジックが`calculation_rules`に集約され、どの科目に、どのシナリオで、
 * どの期間に適用されるかが一意に定まる。
 * - 計算ロジックの詳細はJSONB型に格納し、将来的な拡張性と柔軟性を担保する。
 * JSONB内部では、勘定科目をIDで厳密に参照しつつ、人間向けの検証性のために科目名も冗長的に保持する。
 ****************************************************************************************/
