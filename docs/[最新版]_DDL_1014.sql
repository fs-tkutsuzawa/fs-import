...
CREATE TABLE public.user_accounts (
	...
	CONSTRAINT user_accounts_fs_type_check CHECK (((fs_type)::text = ANY (ARRAY[('BS'::character varying)::text, ('PL'::character varying)::text, ('CF'::character varying)::text, ('PPE'::character varying)::text]))),
...
CREATE TABLE public.calculation_rules (
	...
	CONSTRAINT calculation_rules_rule_type_check CHECK (((rule_type)::text = ANY (ARRAY[('PARAMETER'::character varying)::text, ('BALANCE_AND_CHANGE'::character varying)::text]))),
